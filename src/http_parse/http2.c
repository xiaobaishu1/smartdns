...
static int _http2_process_settings_frame(struct http2_ctx *ctx, uint32_t stream_id, const uint8_t *data, int len, uint8_t flags)
{
	...
	while (offset + 6 <= len) {
		...
		case HTTP2_SETTINGS_INITIAL_WINDOW_SIZE:
			 ...
			 * Send WINDOW_UPDATE frames to notify peer of the delta.
			...
			if (delta != 0) {
				...
				list_for_each_entry(stream, &ctx->streams, node) {
					...
					/* Send WINDOW_UPDATE frame for each active stream */
					if (_http2_send_window_update(ctx, stream->stream_id, delta) < 0) {
						tlog(TLOG_DEBUG, "failed to send window update for stream %u", stream->stream_id);
						/* Continue anyway, do not abort connection */
					}
...
int http2_stream_is_end(struct http2_stream *stream)
{
	...
	int stream_ended = (stream->end_stream_received || stream->state == HTTP2_STREAM_CLOSED ||
	                    (!ctx || ctx->status < 0));
	int is_end = stream_ended && (stream->body_read_offset >= stream->body_buffer_len);
