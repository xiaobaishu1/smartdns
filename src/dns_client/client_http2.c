												...
												struct dns_conn_stream *conn_stream)
{
	...
	if (http2_stream_is_end(http2_stream)) {
		...
		if (http2_stream_get_header(http2_stream, "content-type", content_type_buf, sizeof(content_type_buf)) >= 0) {
		...
		if (status == 200 || (status > 0 && is_dns_message)) {
			...
			if (status >= 500) {
				tlog(TLOG_WARN, "http2 server query from %s:%d failed with 5xx error, HTTP status=%d, marking server prohibited",
				     server_info->ip, server_info->port, status);
				server_info->prohibit = 1;
			} else {
				tlog(TLOG_WARN, "http2 server query from %s:%d failed (non-5xx), HTTP status=%d, Content-Type=%s, response discarded, keep connection",
				     server_info->ip, server_info->port, status,
