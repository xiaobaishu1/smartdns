...
impl DB {
    ...
    pub fn get_domain_top_list(&self, count: u32) -> Result<Vec<DomainQueryCount>, Box<dyn Error>> {
        ...
        let mut stmt = conn.prepare("SELECT domain, count, timestamp_start, timestamp_end FROM top_domain_list ORDER BY count DESC LIMIT ?")?;
        let rows = stmt.query_map([count.to_string()], |row| {
            Ok(DomainQueryCount {
                domain: row.get(0)?,
                count: row.get(1)?,
    ...
    pub fn get_domain_top_blocked_list(&self, count: u32) -> Result<Vec<DomainQueryCount>, Box<dyn Error>> {
        ...
        let mut stmt = conn.prepare("SELECT domain, count, timestamp_start, timestamp_end FROM top_domain_blocked_list ORDER BY count DESC LIMIT ?")?;
        let rows = stmt.query_map([count.to_string()], |row| {
            Ok(DomainQueryCount {
                domain: row.get(0)?,
                count: row.get(1)?,
    ...
    pub fn get_last_query_timestamp_by_mac(&self, mac: &str) -> Result<Option<u64>, Box<dyn Error>> {
        ...
        let conn = conn.as_ref().unwrap();
        let mut stmt = conn.prepare("SELECT MAX(last_query_timestamp) FROM client WHERE mac = ?")?;
    ...
    pub fn delete_client_by_mac(&self, mac: &str) -> Result<u64, Box<dyn Error>> {
        ...
        let conn = conn.as_ref().unwrap();
        let ret = conn.execute("DELETE FROM client WHERE mac = ?", &[&mac]);
        if let Err(e) = ret {
            return Err(Box::new(e));
        }
        Ok(ret.unwrap() as u64)
